...
bool Parser::isBinary(const QString &filename)
{
    ...
    if (file.open(QIODevice::ReadOnly) && file.getChar(&firstByte))
    {
        ...
        {
            char nextChar;
            if (file.getChar(&nextChar) && nextChar == '\xBB' && file.getChar(&nextChar) && nextChar == '\xBF')
            {
                // UTF-8 text file
                return false;
            }
...
bool Parser::isUtf8(const char *string)
{
    ...
    while (*bytes)
    {
        ...
        if (( // excluding overlongs
            ...
            ( // straight 3-byte
                ((0xE1 <= bytes[0] && bytes[0] <= 0xEC) || bytes[0] == 0xEE || bytes[0] == 0xEF) && (0x80 <= bytes[1] && bytes[1] <= 0xBF) &&
        ...
        if (( // planes 1-3
            ...
            ( // planes 4-15
                (0xF1 <= bytes[0] && bytes[0] <= 0xF3) && (0x80 <= bytes[1] && bytes[1] <= 0xBF) && (0x80 <= bytes[2] && bytes[2] <= 0xBF) &&
                (0x80 <= bytes[3] && bytes[3] <= 0xBF)) ||
            ( // plane 16
...
TrackFile Parser::playlistEntryToTrackFile(const QString &playlistEntry, const QString &basePath)
{
